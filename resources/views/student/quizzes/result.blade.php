    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white dark:bg-stone-800 overflow-hidden shadow-sm sm:rounded-lg mb-6">
                <div class="p-6">
                    <div class="text-center">
                        <h3 class="text-2xl font-bold text-stone-900 dark:text-stone-100 mb-2">
                        ...
                        @if($attempt->status === 'timed_out')
                            <p class="mt-2 text-red-600 dark:text-red-400 font-medium">
                                Time expired - quiz was submitted automatically
                            </p>
                        @endif
                    </div>
                </div>
            ...
            <div class="bg-white dark:bg-stone-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <h3 class="text-lg font-medium text-stone-900 dark:text-stone-100 mb-4">Question Details</h3>
                    
                    @foreach($attempt->quiz->questions as $index => $question)
                        <div class="mb-6 p-4 border border-stone-200 dark:border-stone-700 rounded-lg @if($attempt->answers->where('question_id', $question->id)->first()->is_correct ?? false) bg-green-50 dark:bg-green-900 @else bg-red-50 dark:bg-red-900 @endif">
                            ...
                            @elseif($question->type === 'fill_blank')
                                <div class="space-y-2">
                                    <div class="p-2 rounded bg-stone-100 dark:bg-stone-700">
                                        <span class="text-stone-700 dark:text-stone-300">
            ...
            <!-- Back Button -->
            <div class="mt-6">
                <a href="{{ route('student.quizzes.index') }}" class="inline-flex items-center px-4 py-2 bg-stone-600 dark:bg-stone-700 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-stone-700 dark:hover:bg-stone-600">
                    Back to Quizzes
                </a>
            </div>
        </div>
