        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 bg-white border-b border-gray-200">
                <div class="flex justify-between items-center mb-6">
                    <h2 class="text-2xl font-semibold text-gray-800">تفاصيل الدفع - {{ $student->name }}</h2>
                                    ...
                                    @if($student->paid_at)
                                        ...
                                        @if($isExpired)
                                            <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-red-100 text-red-800">
                                                انتهت صلاحية الدفع ({{ $expiryDate->format('M d, Y') }})
                                            </span>
                                        @else
                                            <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">
                                                تم الدفع ({{ $paymentDate->format('M d, Y') }})
                                            </span>
                                            <p class="mt-1 text-sm text-gray-500">
                                                تنتهي الصلاحية خلال {{ $daysRemaining }} يومًا ({{ $expiryDate->format('M d, Y') }})
                                            </p>
                                        @endif
                                    @else
                        ...
                        <div class="flex">
                            ...
                            ">
                                <p class="text-sm text-yellow-700">
                                    حالة دفع هذا الطالب {{ $student->paid_at ? 'منتهية الصلاحية' : 'غير مسجلة' }}.
                                    يُرجى تسجيل الدفع عند استلامه.
                                </p>
                                <div class="mt-4">
