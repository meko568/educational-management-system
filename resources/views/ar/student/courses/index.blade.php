        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
                        ...
                        <div class="flex items-start">
                            <div class="flex-1">
                                @if($student->hasValidSubscription())
                                    <h3 class="text-lg font-semibold text-green-800 dark:text-green-200">✓ اشتراك نشط</h3>
                                    <p class="text-sm text-green-700 dark:text-green-300 mt-1">
                                        اشتراكك صالح حتى: <span class="font-semibold">{{ $student->getSubscriptionExpiryDate()->format('F d, Y') }}</span>
                                    </p>
                                    <p class="text-xs text-green-600 dark:text-green-400 mt-2">لديك وصول كامل إلى جميع محتويات الدورة</p>
                                @else
                                    <h3 class="text-lg font-semibold text-red-800 dark:text-red-200">✗ لا يوجد اشتراك نشط</h3>
                                    <p class="text-sm text-red-700 dark:text-red-300 mt-1">انتهت صلاحية اشتراكك أو لم تقم بالدفع بعد.</p>
                                    <p class="text-xs text-red-600 dark:text-red-400 mt-2">يرجى التواصل مع معلمك أو التحقق من حالة الدفع في حسابك.</p>
                                @endif
                            </div>
                        </div>
                    </div>
                    ...
                    @if($courses->isEmpty())
                        <p class="text-gray-500 dark:text-gray-400 text-center py-8">لا توجد دورات متاحة بعد.</p>
                    @else
                                ...
                                <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-6 border border-gray-200 dark:border-gray-600 hover:shadow-lg transition">
                                    <div class="mb-4">
                                        <span class="inline-block px-2 py-1 text-xs font-semibold text-blue-700 dark:text-blue-200 bg-blue-100 dark:bg-blue-900 rounded">
                                            لديك وصول كامل إلى جميع محتويات الدورة
                                        </span>
                                    </div>
                                    ...
                                    @endif
                                    <p class="text-xs text-gray-500 dark:text-gray-400 mb-4">{{ $course->lessons_count }} درس</p>
