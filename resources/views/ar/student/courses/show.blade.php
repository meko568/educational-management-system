        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
                    <div class="mb-6">
                        <p class="text-sm text-gray-500 dark:text-gray-400 mb-2">كود الدورة: {{ $course->code }}</p>
                    ...
                    @if(!$hasAccess)
                        <div class="mb-6 p-4 bg-yellow-50 dark:bg-yellow-900 border border-yellow-200 dark:border-yellow-700 rounded-lg">
                            <div class="flex items-start">
                                <div class="flex-1">
                                    <h3 class="text-sm font-semibold text-yellow-800 dark:text-yellow-200">⚠️ وصول محدود</h3>
                                    <p class="text-sm text-yellow-700 dark:text-yellow-300 mt-1">انتهت صلاحية اشتراكك. لا يمكنك الوصول إلى محتوى الدروس (الفيديوهات وملفات PDF).</p>
                                    <p class="text-xs text-yellow-600 dark:text-yellow-400 mt-2">يرجى التواصل مع معلمك لتجديد اشتراكك.</p>
                                </div>
                            </div>
                        </div>
                    @else
                        <div class="mb-6 p-4 bg-green-50 dark:bg-green-900 border border-green-200 dark:border-green-700 rounded-lg">
                            <p class="text-sm text-green-700 dark:text-green-300">✓ لديك وصول إلى جميع المحتويات حتى <span class="font-semibold">{{ $student->getSubscriptionExpiryDate()->format('F d, Y') }}</span></p>
                        </div>
                    @endif

                    <div class="mt-8">
                        <h3 class="text-lg font-semibold text-gray-800 dark:text-gray-200 mb-4">الدروس ({{ $lessons->count() }})</h3>

                        @if($lessons->isEmpty())
                            <p class="text-gray-500 dark:text-gray-400 text-center py-8">لا توجد دروس متاحة في هذه الدورة بعد.</p>
                        @else
                            <div class="space-y-3">
