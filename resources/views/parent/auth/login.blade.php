    ...
    <div class="space-y-8">
        <div>
            <h2 class="text-center text-3xl font-extrabold text-stone-900 dark:text-white">
                {{ __('messages.parent_login_title') }}
            </h2>
        </div>
        ...
        <form class="space-y-6" method="POST" action="{{ route('parent.login.store') }}">
            @csrf

            <div class="space-y-4">
                <div>
                    <label for="code" class="block text-sm font-medium text-stone-700 dark:text-stone-300 mb-1" style="color: var(--text-main); font-weight: 700;">{{ __('messages.parent_code') }}</label>
                    <input
                        id="code"
                        name="code"
                        type="text"
                        ...
                        placeholder="{{ __('messages.parent_code_placeholder') }}"
                    >
                </div>
        ...
        <div class="text-center text-sm">
            <p class="text-stone-600 dark:text-stone-500 mt-2">
                <a href="{{ route('login') }}" class="font-bold text-orange-600 dark:text-orange-400 hover:text-orange-500 transition-colors">
                    {{ __('messages.student_login_link') }}
                </a>
            </p>
        </div>
    </div>
