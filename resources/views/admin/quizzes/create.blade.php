<x-app-layout>
    <div style="display: flex; flex-direction: column; gap: 2rem; max-width: 42rem; margin: 0 auto;">
        ...
        <div style="display: flex; flex-direction: column; gap: 1rem; justify-content: space-between; align-items: flex-start;" class="md:flex-row md:items-center">
            <div>
                <h1 style="font-size: 1.5rem; font-weight: 700; color: var(--text-main); margin: 0;">Create New Manual Quiz</h1>
                <p style="color: var(--text-muted); margin-top: 0.25rem; font-size: 0.875rem;">Set up an offline quiz for {{ strtoupper($academicYear ?? 'primary1') }}</p>
            </div>
        ...
        <div class="card-custom">
            <form method="POST" action="{{ route('admin.manual-quizzes.store') }}" style="display: flex; flex-direction: column; gap: 1.5rem;">
                @csrf
                ...
                <div style="display: flex; flex-direction: column; gap: 0.5rem;">
                    ...
                    <input id="title" name="title" type="text" value="{{ old('title') }}" required autofocus
                           style="width: 100%; padding: 0.75rem 1rem; background-color: var(--bg-alt); border: 1px solid var(--border-color); border-radius: 0.75rem; color: var(--text-main); font-size: 0.875rem;" placeholder="Quiz Chapter 1, etc.">
                ...
                <div style="margin-top: 1rem;">
                    <button type="submit" style="width: 100%; padding: 1rem; background-color: #ea580c; color: white; border: none; border-radius: 1rem; font-weight: 800; font-size: 0.875rem; text-transform: uppercase; letter-spacing: 0.05em; cursor: pointer; box-shadow: 0 10px 15px -3px rgba(234, 88, 12, 0.2);">
                        Create Quiz Record
                    </button>
                </div>
            </form>
