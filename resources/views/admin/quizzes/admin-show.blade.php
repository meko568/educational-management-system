    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <div class="flex space-x-2">
                ...
                @else
                    <a href="{{ route('admin.quizzes.questions.create', $quiz->id) }}" class="inline-flex items-center px-4 py-2 bg-orange-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-orange-700">
                        Add Questions
                    </a>
                @endif
            </div>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                ...
                <div class="p-6">
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <div>
                            ...
                            <p class="mt-1 text-sm text-stone-900 dark:text-stone-100 font-semibold">{{ $quiz->grade }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-stone-500 dark:text-stone-400">Duration</label>
                            <p class="mt-1 text-sm text-stone-900 dark:text-stone-100 font-semibold">{{ $quiz->duration_minutes }} minutes</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-stone-500 dark:text-stone-400">Start Time</label>
                            <p class="mt-1 text-sm text-stone-900 dark:text-stone-100">{{ $quiz->start_datetime->format('M d, Y - H:i') }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-stone-500 dark:text-stone-400">End Time</label>
                            <p class="mt-1 text-sm text-stone-900 dark:text-stone-100">{{ $quiz->end_datetime->format('M d, Y - H:i') }}</p>
                        </div>
                    </div>
                    @if($quiz->description)
                        <div class="mt-4">
                            <label class="block text-sm font-medium text-stone-500 dark:text-stone-400">Description</label>
                            <p class="mt-1 text-sm text-stone-900 dark:text-stone-100">{{ $quiz->description }}</p>
                        </div>
                    @endif
                </div>
            </div>
                ...
                <div class="p-6">
                    ...
                    @else
                        <div class="text-center py-8 text-stone-500 dark:text-stone-400">
                            ...
                            <a href="{{ route('admin.quizzes.questions.create', $quiz->id) }}" class="text-orange-600 hover:text-orange-900 dark:text-orange-400">Add questions now</a>
                        </div>
                    @endif
                </div>
            </div>
            ...
            <div class="flex justify-between">
                <a href="{{ route('admin.quizzes.index') }}" class="inline-flex items-center px-4 py-2 bg-stone-600 dark:bg-stone-700 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-stone-700 dark:hover:bg-stone-600">
                    Back to Quizzes
                </a>
                <div class="flex space-x-2">
                    <a href="{{ route('admin.quizzes.edit', $quiz->id) }}" class="inline-flex items-center px-4 py-2 bg-orange-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-orange-700">
                        Edit Quiz
                    </a>
