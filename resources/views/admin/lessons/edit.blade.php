<x-app-layout>
    <div style="display: flex; flex-direction: column; gap: 2rem; max-width: 48rem; margin: 0 auto;">
        ...
        <div style="display: flex; flex-direction: column; gap: 1rem; justify-content: space-between; align-items: flex-start;" class="md:flex-row md:items-center">
            <div>
                <h1 style="font-size: 1.5rem; font-weight: 700; color: var(--text-main); margin: 0;">Edit Lesson</h1>
                <p style="color: var(--text-muted); margin-top: 0.25rem; font-size: 0.875rem;">Modifying: <span style="font-weight: 700; color: var(--accent-color);">{{ $lesson->title }}</span></p>
            </div>
        ...
        <div class="card-custom">
            <form method="POST" action="{{ route('admin.courses.lessons.update', [$academicYear, $course, $lesson]) }}" style="display: flex; flex-direction: column; gap: 2rem;">
                @csrf
                @method('PUT')
                ...
                <div style="display: flex; flex-direction: column; gap: 1rem;">
                    ...
                    <div style="display: flex; flex-direction: column; gap: 0.5rem;">
                        ...
                        <input id="scheduled_date" name="scheduled_date" type="date" required
                               ...
                               value="{{ old('scheduled_date', $lesson->scheduled_date->format('Y-m-d')) }}"
                ...
                <div style="display: flex; flex-direction: column; gap: 0.5rem;">
                    ...
                    <input id="title" name="title" type="text" value="{{ old('title', $lesson->title) }}" required autofocus
                ...
                <div style="display: flex; flex-direction: column; gap: 0.5rem;">
                    ...
                    <textarea id="description" name="description" rows="3"
                              style="width: 100%; padding: 0.75rem 1rem; background-color: var(--bg-alt); border: 1px solid var(--border-color); border-radius: 0.75rem; color: var(--text-main); font-size: 0.875rem; resize: vertical;">{{ old('description', $lesson->description) }}</textarea>
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem;">
                    ...
                    <div style="display: flex; flex-direction: column; gap: 0.5rem;">
                        ...
                        <input id="pdf_url" name="pdf_url" type="url" value="{{ old('pdf_url', $lesson->pdf_url) }}"
                ...
                <div style="display: flex; flex-direction: column; gap: 0.5rem;">
                    ...
                    <input id="order" name="order" type="number" value="{{ old('order', $lesson->order) }}" min="0" required
                           style="width: 100%; padding: 0.75rem 1rem; background-color: var(--bg-alt); border: 1px solid var(--border-color); border-radius: 0.75rem; color: var(--text-main); font-size: 0.875rem;">
