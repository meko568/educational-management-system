<x-app-layout>
    <div style="display: flex; flex-direction: column; gap: 2rem; max-width: 48rem; margin: 0 auto;">
        ...
        <div style="display: flex; flex-direction: column; gap: 1rem; justify-content: space-between; align-items: flex-start;" class="md:flex-row md:items-center">
            <div>
                <h1 style="font-size: 1.5rem; font-weight: 700; color: var(--text-main); margin: 0;">Edit Auto-Revision Exam</h1>
                <p style="color: var(--text-muted); margin-top: 0.25rem; font-size: 0.875rem;">Modifying: <span style="font-weight: 700; color: #db2777;">{{ $exam->title }}</span></p>
            </div>
        ...
        <div class="card-custom">
            <form method="POST" action="{{ route('admin.exams.update', $exam->id) }}" style="display: flex; flex-direction: column; gap: 2rem;">
                @csrf
                @method('PUT')
                ...
                <div style="display: flex; flex-direction: column; gap: 1rem;">
                    <label style="font-size: 0.875rem; font-weight: 700; color: var(--text-main);">Reschedule Exam Day</label>
                ...
                <div style="display: flex; flex-direction: column; gap: 0.5rem;">
                    ...
                    <input id="duration_minutes" name="duration_minutes" type="number" value="{{ old('duration_minutes', $exam->duration_minutes) }}" min="1" required
                           style="width: 100%; padding: 0.75rem 1rem; background-color: var(--bg-alt); border: 1px solid var(--border-color); border-radius: 0.75rem; color: var(--text-main); font-size: 0.875rem;">
